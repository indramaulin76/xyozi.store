...
$(document).ready(function () {
    var table = $('#produkTable').DataTable({
        ...
        "pagingType": 'simple',
        "ajax": {
            "url": "<?php echo base_url('admin/mlbblayanan/getMlbbProduk'); ?>",
            "type": "POST"
        },
        "columns": [
            ...
            {"data": null, "render": function (data, type, row) {
                return '<div class="flex">' +
                    '<a href="<?= base_url('admin/mlbblayanan/hapus/') ?>' + row.id + '" type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-sm px-3 py-1 me-2" data-id="' + row.id + '">Hapus</a></div>';
            }}
        ]
